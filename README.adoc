...
this guide, you will use the Jayway JsonPath library, that already comes with Spring HateOAS.
...
`linkTo(…)` and `methodOn(…)` are static methods on `WebMvcLinkBuilder` that let you
