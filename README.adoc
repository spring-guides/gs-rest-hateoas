...
If you use Maven, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.3.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=rest-hateoas&name=rest-hateoas&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.rest-hateoas&dependencies=hateoas[Spring Initializr] to generate a new project with the required dependency (Spring HATEOAS).
