...
:MappingJackson2HttpMessageConverter: http://docs.spring.io/spring/docs/4.0.0.RC1/javadoc-api/org/springframework/http/converter/json/MappingJackson2HttpMessageConverter.html
:toc:
:icons: font
:source-highlighter: prettify
:project_id: gs-rest-hateoas

This guide walks you through the process of creating a "hello world" link:/understanding/REST[Hypermedia Driven REST web service] with Spring.
...
In Spring's approach to building RESTful web services, HTTP requests are handled by a controller. The components are easily identified by the {Controller}[`@Controller`] annotation, and the `GreetingController` below handles `GET` requests for `/greeting` by returning a new instance of the `Greeting` class:
...
The {ResponseBody}[`@ResponseBody`] annotation on the `greeting` method will cause Spring MVC to render the returned `HttpEntity` and its payload, the `Greeting`, directly to the response.
