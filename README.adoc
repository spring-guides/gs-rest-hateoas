...
:wikipedia-hal: https://en.wikipedia.org/wiki/Hypertext_Application_Language#:~:text=HAL%20was%20created,%20HAL%2E
...
:x-forwarded-headers: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers#Proxies
...
:hateoas-abbr: pass:[<abbr title="Hypermedia As The Engine Of Application State">HATEOAS</abbr>]

You will build a hypermedia-driven REST service with Spring {hateoas-abbr}: a library of APIs
...
an application and does a lot of the setup for you. This example needs the Spring HATEOAS
dependency.
...
this guide, you will use the {jayway}[Jayway JsonPath] library; it's already included as a 
dependency for you by the `spring-boot-starter-test` dependency.
...
to the response—as opposed to treating it like a page and wrapping it in HTML.
...
NOTE: Spring HATEOAS respects various {x-forwarded-headers}[`X-FORWARDED-` headers]. If you put a Spring HATEOAS
...
We're practically ready to run the application. Here's the code and some explanation for the main starting point of the app.
====
[source,java]
----
include::complete/src/main/java/com/example/resthateoas/RestHateoasApplication.java[]
----
====
...
For a more in-depth look at Spring HATEOAS, check out

*  the https://spring.io/guides/tutorials/rest/[Building REST services with Spring] guide or 
* the short talk https://www.youtube.com/watch?v=nVEmfmdDUXU[Building Flexible APIs with Spring HATEOAS] from SpringOne 2020 or
* the https://spring.io/projects/spring-hateoas#samples[Spring HATEOAS Project page].
