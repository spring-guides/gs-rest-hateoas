...
can also use Gradle. It also shows values of `com.example` and `rest-hateoas` as the Group
...
because, by default, `@RequestMapping` maps all HTTP operations. Use
`@RequestMapping(path="/greeting", method=RequestMethod.GET)` to narrow this mapping. In
that case, you also want to
